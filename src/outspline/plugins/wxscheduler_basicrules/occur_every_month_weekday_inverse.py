...
from outspline.static.wxclasses.time import (HourCtrl, MonthWeekdayHourCtrl,
                                     MonthInverseWeekdayHourCtrl, TimeSpanCtrl)
...
_RULE_DESC = 'Occur on the n-th-to-last weekday of every month'
...
class Rule():
    ...
    def _create_widgets_start(self):
        ...
        self.startw = MonthInverseWeekdayHourCtrl(self.mpanel)
        self.startw.set_values(self.original_values['rstartin'],
    ...
    def apply_rule(self, filename, id_):
        ...
        rstartin = self.startw.get_weekday_number()
        ...
        try:
            ruled = organism_basicrules_api.make_occur_monthly_weekday_inverse_rule(
                              smonths, weekday, rstartin, rstart, rend, ralarm,
                                                    ('1m', endtype, alarmtype))
        except organism_basicrules_api.BadRuleError:
            msgboxes.warn_bad_rule(msgboxes.generic).ShowModal()
        else:
            label = self._make_label(rstartin, rstartA, rstartH, rstartM, rendH,
    ...
    def insert_rule(cls, filename, id_, rule, rulev):
        ...
        label = cls._make_label(values['rstartin'], values['rstartA'],
                                          values['rstartH'], values['rstartM'],
    ...
    def _make_label(rstartin, rstartA, rstartH, rstartM, rendH, rendM, ralarmH,
        ...
        label = 'Occur on the {} {} of every month at {}:{}'.format(
                     MonthInverseWeekdayHourCtrl._compute_weekday_number_label(
                                                            rstartin), rstartA,
    ...
    def create_random_rule():
        ...
        return organism_basicrules_api.make_occur_monthly_weekday_inverse_rule(
                               smonths, weekday, inumber, rstart, rend, ralarm,
                                                    ('1m', endtype, alarmtype))
