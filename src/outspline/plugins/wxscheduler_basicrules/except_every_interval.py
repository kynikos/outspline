...
class Rule(object):
    ...
    def apply_rule(self, filename, id_):
        ...
        try:
            if standard == 'UTC':
                ruled = organism_basicrules_api.make_except_regularly_single_rule_UTC(
                              refstart, interval, rend, inclusive, (endtype, ))
            else:
                ruled = organism_basicrules_api.make_except_regularly_single_rule_local(
                              refstart, interval, rend, inclusive, (endtype, ))
    ...
    def insert_rule(cls, filename, id_, rule, rulev):
        standard = 'UTC' if rule['rule'] == 'except_regularly_single_UTC' \
                                                                else 'local'
    ...
    def create_random_rule():
        ...
        if stdn == 0:
            return organism_basicrules_api.make_except_regularly_single_rule_local(
                              refstart, interval, rend, inclusive, (endtype, ))
        else:
            return organism_basicrules_api.make_except_regularly_single_rule_UTC(
                              refstart, interval, rend, inclusive, (endtype, ))
